// Edited by Jay, 2022.
// Source: Unity built-in shader source. Copyright (c) 2016 Unity Technologies. MIT license (see license.txt)

Shader "Skybox/ProceduralWithMoon" {
    Properties{
        [KeywordEnum(None, Simple, High Quality)] _SunDisk("Sun & Moon", Int) = 2
        [Header(Sun)][Space]
        _SunSize("Sun Size", Range(0,1)) = 0.04
        _SunTint("Sun Tint", Color) = (1, .95, .9, 1)
        _SunSizeConvergence("Sun Size Convergence", Range(1,10)) = 5
        [Header(Moon)][Space]
        _MoonSize("Moon Size", Range(0,1)) = 0.1
        _MoonTint("Moon Tint", Color) = (1, .95, .9, 1)
        _MoonSizeConvergence("Moon Size Convergence", Range(1,10)) = 5
        [Space]
        _AtmosphereThickness("Atmosphere Thickness", Range(0,5)) = 1.0
        _SkyBase("Sky Base", Color) = (.01, .01, .1, 1)
        ...
        SubShader{
            ...
            Pass {
                ...
                uniform half3 _MoonTint;
                uniform half _MoonSizeConvergence;

                uniform half3 _SkyBase;
                ...
                #define kMOON_BRIGHTNESS 20.0    // Moon brightness
                ...
                static const half kHDMoondiskIntensityFactor = 30.0;
                static const half kSimpleMoondiskIntensityFactor = 30.0;

                static const half kSunScale = 400.0 * kSUN_BRIGHTNESS;
                static const float kKmESun = kMIE * kSUN_BRIGHTNESS;

                static const half kMoonScale = 500.0 * kSUN_BRIGHTNESS;
                static const float kKmEMoon = kMIE * kMOON_BRIGHTNESS;
                ...
                v2f vert(appdata_t v)
                {
                    ...
                    float kKrEMoon = kRAYLEIGH * kMOON_BRIGHTNESS;
                    ...
                    OUT.skyColor = _Exposure * (cIn * getRayleighPhase(_WorldSpaceLightPos0.xyz, -eyeRay));
                        ...
                        half lightColorIntensity = clamp(length(_LightColor0.xyz), 0.25, 1);
                            ...
                            OUT.sunColor = _SunTint * kSimpleSundiskIntensityFactor * saturate(cOut * kSunScale) * _LightColor0.xyz / lightColorIntensity;
                            // OUT.moonColor = kSimpleMoondiskIntensityFactor * saturate(cOut * kMoonScale) * _LightColor0.xyz / lightColorIntensity;
                            OUT.moonColor = _MoonTint * kSimpleMoondiskIntensityFactor * kMoonScale;
                        #else // SKYBOX_SUNDISK_HQ
                            OUT.sunColor = _SunTint * kHDSundiskIntensityFactor * saturate(cOut) * _LightColor0.xyz / lightColorIntensity;
                            //OUT.moonColor = kHDMoondiskIntensityFactor * saturate(cOut) * _LightColor0.xyz / lightColorIntensity;
                            OUT.moonColor = _MoonTint * kHDMoondiskIntensityFactor; // *saturate(cOut);
                        #endif
                    #endif
                ...
                half getMiePhaseForMoon(half eyeCos, half eyeCos2)
                {
                    half temp = 1.0 + MIE_G2 - 2.0 * MIE_G * eyeCos;
                ...
                // Calculates the sun shape
                half calcMoonAttenuation(half3 lightPos, half3 ray)
                {
                #if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
                    half3 delta = lightPos - ray;
                    half dist = length(delta);
                    ...
                    return getMiePhaseForMoon(-focusedEyeCos, focusedEyeCos * focusedEyeCos);
                #endif
                }
                ...
                half4 frag(v2f IN) : SV_Target
                {
                    ...
                    col = lerp(IN.skyColor, IN.groundColor, saturate(y)) + _SkyBase;

                    #if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
                        if (y < 0.0)
                        {
                            col += IN.sunColor * calcSunAttenuation(_WorldSpaceLightPos0.xyz, -ray);
                            col += IN.moonColor * calcMoonAttenuation(-_WorldSpaceLightPos0.xyz, -ray);
                        }
                    #endif
